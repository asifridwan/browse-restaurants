...
export default function Saved({userID, back}) {
    ...
    useEffect(() => {
        axios.get(`https://browse-restaurants-api.herokuapp.com/collections/${userID}`).then(res => setCollections(res.data))
    ...
    function ConfirmRename() {
        axios.patch(`https://browse-restaurants-api.herokuapp.com/rename/${collectionID}`, {
            name: collectionName
        }).then(response => {
    ...
    function ConfirmDeleteCollection() {
        axios.delete(`https://browse-restaurants-api.herokuapp.com/collection/${collectionID}`).then(response => {
            if (response.data === 'Success') {
                CloseDelete();
            }
        });
    ...
    function ConfirmDeleteRestaurant() {
        axios.delete(`https://browse-restaurants-api.herokuapp.com/restaurant/${restaurantID}`).then(response => {
            if (response.data === 'Success') {
                CloseDelete();
            }
        });
